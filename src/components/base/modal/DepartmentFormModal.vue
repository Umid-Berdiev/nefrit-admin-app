...
watch(
  ...
  async (newVal) => {
    if (newVal) {
      ...
      Object.assign(
        departmentData.stages,
        res.stages.map(({ id }) => id)
      )
    }
  },
...
async function onSubmit(event: Event) {
  try {
    ...
    props.departmentId
      ? await updateById(props.departmentId, departmentData)
        ...
        <div class="columns is-multiline">
          <div class="column is-12">
            <VField :label="$t('Name_uz')" required>
              <VControl>
                <VInput
                  v-model="departmentData.name.uz"
                  name="name_uz"
            ...
            <VField :label="$t('Name_ru')">
              <VControl>
                <VInput
                  v-model="departmentData.name.ru"
                  name="name_ru"
            ...
            <VField :label="$t('Name_en')">
              <VControl>
                <VInput
                  ...
                  v-model="departmentData.name.en"
                />
              </VControl>
            </VField>
          </div>
