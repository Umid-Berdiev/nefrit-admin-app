...
import { create, updateById, fetchById } from '/@src/utils/api/doc-category';
...
watch(
  ...
  async (newVal) => {
    if (newVal) {
      title.value = t('Edit')
      const res = await fetchById(Number(props.itemId))
...
async function onSubmit(event: Event) {
  try {
    ...
    props.itemId ? await updateById(props.itemId, { name, description }) : await create({ name, description })
    emits('update:list')
    onClose()
  } catch (error: any) {
...
<template>
  <VModal :open="modelValue" size="large" :title="title" actions="right" @close="onClose">
    ...
    <template #action="{ close }">
      <VButton type="submit" color="primary" outlined form="role-form" :loading="isLoading" :disabled="isLoading">
        {{ $t('Save') }}
      </VButton>
    </template>
  </VModal>
