...
onMounted(async () => {
  ...
  selectedStage.value = props.statementStage
...
async function onSubmit(event: Event) {
  try {
    ...
    await updateStatementStage(Number(props.statementId), { stage_id: selectedStage.value })
    emits('update:data')
    onClose()
  } catch (error: any) {
...
<template>
  <VModal :open="modelValue" size="large" :title="title" actions="right" @close="onClose">
    <template #content>
      <form id="statement-stage-form" class="modal-form" @submit.prevent="onSubmit" style="min-height: 200px;">
        <div class="columns is-multiline">
          <div class="column is-12">
            <VField :label="$t('Stages')">
              <VControl>
                <Multiselect v-model="selectedStage" :options="stagesList" :placeholder="$t('Select_stages')"
                  valueProp="id" label="name" />
    ...
    <template #action="{ close }">
      <VButton :loading="isLoading" color="primary" outlined type="submit" form="statement-stage-form"
        :disabled="isLoading">
        {{ $t('Save') }}
      </VButton>
    </template>
